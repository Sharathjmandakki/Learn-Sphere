    ...
    useEffect(() => {
        ...
        const getAllCourses = async () => {
            try {  
                ...
                const response = await axios.get("http://localhost:8080/purchased");
                setCourse(response.data)
                if(response.data.length==0){                    
                    document.getElementById("box").innerHTML="<div class='min-h-96 m-24 text-4xl text-slate-400'>Not Purchased Yet 🥲</div>"
                }
            } catch (e) {
                document.getElementById("box").innerHTML="<div class='min-h-96 m-24 p-10 text-4xl text-slate-400'>404 Error <div className='flex justify-center'><hr/><a href='/entry' class='text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800'>Go to Login page</a></div></div>"
