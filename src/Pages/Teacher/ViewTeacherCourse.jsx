...
export default function ViewTeacherCourse() {
              ...
              <h1 class="flex-1 text-zinc-100 ms-3 text-center text-2xl font-bold whitespace-nowrap overflow-hidden ">📓 Lessons </h1><br /><hr /></li>
            {
              data && data.lessons && data.lessons.map((e) => (
                <li className='mb-2 relative'>
                  <button onClick={() => lesson(e)} class="min-w-32 block w-full items-center p-5 text-base font-bold text-gray-900 rounded-lg bg-gray-50 hover:bg-gray-100 group hover:shadow dark:bg-gray-600 dark:hover:bg-gray-500 dark:text-white">
                    <span class="flex flex-nowrap ms-3 whitespace-nowrap overflow-hidden">{e.name}</span>
                  </button>
                  <button onClick={()=>{editlesson(e)}} type="submit" class="text-white absolute end-2.5 bottom-2.5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-lg px-4 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">edit</button>
                </li>
              ))
            }
          </ul>
